...
const Reports = () => {
  ...
  useEffect(() => {
    ...
    ]).then(([tasksRes, expensesRes, issuesRes]) => {
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-semibold">
                        {Math.round((report.expenses.approved / report.project.budget) * 100)}%
                      </span>
                    </div>
                    <div className="w-full h-1.5 bg-gray-200 rounded-full">
                      <div className="h-1.5 bg-green-500 rounded-full" style={{
                        width: `${Math.min(100, Math.round((report.expenses.approved / report.project.budget) * 100))}%`
                      }} />
                    </div>
                  </div>
                </>
